...
function localizeRoutes(routes, config) {
  ...
  if (config.compactRoutes && strategy !== "no_prefix" && !config.differentDomains && !config.multiDomainLocales && !(strategy === "prefix" && config.includeUnprefixedFallback)) {
  ...
  nuxt.hook(
    ...
    async (pages) => {
      ...
      if (shouldLocalizeRoutes(localizationOptions)) {
        buildPathToConfig(ctx, localeCodes, resolver, pages);
      }
...
function createPureOptionsResolver(ctx, defaultLocale, customRoutes) {
...
function buildPathToConfig(ctx, localeCodes, resolver, routes) {
...
function getDefineConfig({ options, fullStatic, localeHashes }, server = false, nuxt = useNuxt()) {
