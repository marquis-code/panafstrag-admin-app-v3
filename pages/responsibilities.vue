...
useHead({ title: 'Responsibilities | PANAFSTRAG' })
...
<template>
  <div class="space-y-12">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
      <div>
        <p class="text-[10px] font-black uppercase tracking-[0.4em] text-gray-400 mb-2">Institutional Mandate</p>
        <h1 class="text-4xl lg:text-6xl font-black tracking-tighter uppercase italic leading-none text-black">
          Responsibilities <span class="not-italic text-gray-400 leading-none">Management.</span>
        </h1>
      </div>
    ...
    <div v-else-if="!(responsibilities as any[])?.length">
      <EmptyState title="NO DATA" message="The institutional mandate sector is currently unpopulated." />
    </div>
        ...
        <table class="w-full text-left border-collapse min-w-[600px]">
          <thead>
            <tr class="bg-gray-50 border-b border-gray-100">
              <th class="px-8 py-6 text-[10px] font-black uppercase tracking-widest text-gray-400">Mandate Description</th>
              <th class="px-8 py-6 text-[10px] font-black uppercase tracking-widest text-gray-400 text-right">Actions</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100">
            <tr v-for="res in (responsibilities as any[])" :key="res._id" class="hover:bg-gray-50/50 transition-colors group">
              <td class="px-8 py-6">
                <p class="text-sm font-medium text-black leading-relaxed group-hover:text-black transition-colors">{{ res.description }}</p>
              </td>
              <td class="px-8 py-6 text-right">
                <div class="flex items-center justify-end gap-3">
                   <button @click="openEdit(res)" class="p-2 hover:bg-black hover:text-white border border-transparent hover:border-black transition-all">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                    </svg>
                  </button>
                  <button @click="handleDelete(res._id)" class="p-2 text-gray-300 hover:text-black hover:bg-white transition-all">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <!-- Modal Redesign -->
    <Modal :show="showModal" :title="editingId ? 'EDIT MANDATE CLAUSE' : 'NEW MANDATE INITIALIZATION'" @close="showModal = false">
      <div class="space-y-8 py-4">
        <section class="space-y-6">
          <h3 class="text-[10px] font-black uppercase tracking-[0.3em] text-gray-300 border-b border-gray-100 pb-2">Mandate Parameters</h3>
          <div>
            <label class="block text-[9px] font-black uppercase tracking-widest text-gray-400 mb-2">Detailed Clause Description</label>
            <textarea v-model="form.description" rows="5" class="w-full p-4 bg-gray-50 border-none outline-none focus:ring-1 focus:ring-black transition-all font-medium text-sm leading-relaxed" placeholder="DEFINE THE INSTITUTIONAL RESPONSIBILITY..."></textarea>
          </div>
        </section>
      </div>
      ...
      <template #actions>
        ...
        <button @click="handleSubmit" :disabled="loading" class="px-12 py-5 bg-black text-white text-[10px] font-black uppercase tracking-widest hover:bg-gray-800 disabled:opacity-30 transition-all">
          {{ loading ? 'INITIALIZING...' : 'AUTHORIZE MANDATE' }}
        </button>
      </template>
    </Modal>
    ...
    <ConfirmModal 
      ...
      message="Are you sure you want to remove this institutional responsibility? This operation is permanent."
