...
useHead({ title: 'Objectives | Admin' })
definePageMeta({
  ...
  <div class="space-y-8 pb-12">
    ...
    <div v-else-if="!(objectives as any[])?.length">
      <EmptyState 
        ...
        message="Get started by adding your first mission objective." 
        @action="openCreate"
      />
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse min-w-[600px]">
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-for="(obj, index) in (objectives as any[])" :key="obj._id" class="hover:bg-gray-50 transition-colors group">
              <td class="px-6 py-6">
              ...
              <td class="px-6 py-6">
                <div class="flex items-start gap-4">
                  ...
                  <p class="text-sm font-medium text-gray-700 leading-relaxed max-w-2xl">{{ obj.description }}</p>
                </div>
              </td>
    ...
    <!-- Edit/Create Modal -->
    <Modal :show="showModal" :title="editingId ? 'Edit Objective' : 'Add Objective'" @close="showModal = false" size="lg">
      ...
      <template #actions>
        <div class="flex items-center justify-end gap-4 w-full px-8 pb-8">
          ...
          >
            ...
            <span v-if="!loading">{{ editingId ? 'Save Changes' : 'Add Objective' }}</span>
          </button>
        </div>
      </template>
    </Modal>
